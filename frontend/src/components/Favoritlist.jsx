...
export default function Favoritlist({ userId, user2Id }) {
  ...
  const listCommonFavMovies = favMovies
    ?.filter((movie1) =>
    ...
    <article>
      {listCommonFavMovies && (
        <>
          ...
          <p>
            Dere har {listCommonFavMovies.length} film(er) felles i
            favorittlisten deres
          </p>
          <ul className="article-card">{listCommonFavMovies}</ul>
        </>
      )}
    </article>
  );
