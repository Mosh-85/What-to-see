...
export default function Commonwishlist({ userId, user2Id }) {
  ...
  const listCommonWishMovies = wishMovies
    ?.filter((movie1) =>
    ...
    <article>
      {listCommonWishMovies && (
        <>
          ...
          <p>
            Dere har {listCommonWishMovies.length} film(er) felles i ønskelisten
            deres
          </p>
          <ul className="article-card">{listCommonWishMovies}</ul>
        </>
      )}
    </article>
  );
